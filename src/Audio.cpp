...
void AUDIO_Sound_Play(Audio::AudioTypeEnum SoundNumber)
{
  ...
  {
    if(SoundNumber == Audio::AudioTypeEnum::AUDIO_DISK)       {Mix_PlayChannel(-1,AUDIO_CHUNK_Disk,0);}
    if(SoundNumber == Audio::AudioTypeEnum::AUDIO_JUMP)       {Mix_PlayChannel(-1,AUDIO_CHUNK_Jump,0);}
    if(SoundNumber == Audio::AudioTypeEnum::AUDIO_STOMP)      {Mix_PlayChannel(-1,AUDIO_CHUNK_Stomp,0);}
    if(SoundNumber == Audio::AudioTypeEnum::AUDIO_KILLED)     {Mix_PlayChannel(-1,AUDIO_CHUNK_Killed,0);}
    if(SoundNumber == Audio::AudioTypeEnum::AUDIO_DING)       {Mix_PlayChannel(-1,AUDIO_CHUNK_Ding,0);}
    if(SoundNumber == Audio::AudioTypeEnum::AUDIO_CLICK)      {Mix_PlayChannel(-1,AUDIO_CHUNK_Click,0);}
    if(SoundNumber == Audio::AudioTypeEnum::AUDIO_GOTPOWERUP) {Mix_PlayChannel(-1,AUDIO_CHUNK_GotPowerUp,0);}
    if(SoundNumber == Audio::AudioTypeEnum::AUDIO_SHOOT)      {Mix_PlayChannel(-1,AUDIO_CHUNK_Shoot,0);}
    if(SoundNumber == Audio::AudioTypeEnum::AUDIO_CRACK)      {Mix_PlayChannel(-1,AUDIO_CHUNK_Crack,0);}
  }
...
void AUDIO_Music_Play(Audio::MusicTypeEnum MusicNumber)
{
  if(GV.VolumeMusic > 0 && GV.MusicEnabled)
  {
    ...
    if(MusicNumber == Audio::MusicTypeEnum::MUSIC_MENU)      {Mix_PlayMusic(BGM_Title,-1);}
    if(MusicNumber == Audio::MusicTypeEnum::MUSIC_OUTDOORS)  {Mix_PlayMusic(BGM_Outdoors,-1);}
    if(MusicNumber == Audio::MusicTypeEnum::MUSIC_INDOORS)   {Mix_PlayMusic(BGM_Indoors,-1);}
